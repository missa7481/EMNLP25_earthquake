...
# Global Constants
# =============================================
JSON_DIR = "2022-fukushima_filtered_25percent"
IMAGE_DIR = "2022-fukushima_filtered_images"
...
def get_location_prompt(tweet, location, place, longitude, latitude):
    with open("prompt_location_JP.txt", "r", encoding='utf-8') as f:
        prompt = f.read()
    return eval(prompt)

def get_event_related_prompt(tweet):
    with open("prompt_event_JP.txt", "r", encoding='utf-8') as f:
        prompt = f.read()
    return eval(prompt)

def get_damage_prompt(tweet, prompt_file):
...
def get_damage_prompt_without_text():
    with open("prompt_imageonly.txt", "r", encoding='utf-8') as f:
        prompt = f.read()
    return eval(prompt)

def get_final_prompt(tweet, text_response, image_response, text_image_response):
    with open("prompt_vote.txt", "r", encoding='utf-8') as f:
        prompt = f.read()
    return eval(prompt)
...
def init_gpt(secret_key_file):
    with open(secret_key_file) as f:
        lines = f.readlines()
        for line in lines:
            if line.split(',')[0].strip() == "openai_key_1":
...
def init_gemini(secret_key_file):
    with open(secret_key_file) as f:
        lines = f.readlines()
        for line in lines:
            if line.split(',')[0].strip() == "gemini_key_4w":
...
def call_gemini(text, image_paths, max_tries=10):
    ...
    for i in range(max_tries):
        try: 
            ...
            if finish_reason == "FinishReason.STOP" or "STOP" in finish_reason:
                cleaned_string = response.text.strip('`').replace('json\n', '', 1)
                print("succeed")
                return cleaned_string
            else:
                print(f"not succes: {finish_reason}")
                continue  
        except Exception as e:
            time.sleep(1)
...
def generate_response(model, index, df):
    ...
    # print(f"{damage_prompt=}")
    # print(f"{damage_prompt_without_tweet_text=}")
...
def main():
    ...
    print(f"{df.shape=}")
    save_interval = 1  # Save every 100 rows
    length = df.shape[0]
